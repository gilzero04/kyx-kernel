...
use ntex::http::header::{HeaderValue, ACCESS_CONTROL_ALLOW_ORIGIN, ACCESS_CONTROL_ALLOW_METHODS, ACCESS_CONTROL_ALLOW_HEADERS, ACCESS_CONTROL_ALLOW_CREDENTIALS, ACCESS_CONTROL_MAX_AGE, ORIGIN, VARY};
        ...
        if is_preflight && allowed {
            if let Some(ref origin_str) = origin {
                if let Ok(hv) = HeaderValue::from_str(origin_str) {
                    let mut res = req.into_response(web::HttpResponse::build(StatusCode::OK).finish());
                    res.headers_mut().insert(ACCESS_CONTROL_ALLOW_ORIGIN, hv);
                    res.headers_mut().insert(ACCESS_CONTROL_ALLOW_METHODS, HeaderValue::from_static("GET, POST, PUT, DELETE, OPTIONS, PATCH"));
                    res.headers_mut().insert(ACCESS_CONTROL_ALLOW_HEADERS, HeaderValue::from_static("Content-Type, Authorization, X-Engine-Secret, X-Requested-With"));
                    res.headers_mut().insert(ACCESS_CONTROL_ALLOW_CREDENTIALS, HeaderValue::from_static("true"));
                    res.headers_mut().insert(ACCESS_CONTROL_MAX_AGE, HeaderValue::from_static("86400"));
