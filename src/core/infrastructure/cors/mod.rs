...
impl CorsManager {
    ...
    pub async fn refresh(&self) -> Result<(), AppError> {
        let key =
            std::env::var("REDIS_CORS_KEY").unwrap_or_else(|_| "config:cors_origins".to_string());

        // 1. Fetch from Database (exclude soft-deleted)
        let rows = sqlx::query(
            "SELECT origin FROM sys_cors_origins WHERE is_active = TRUE AND deleted_at IS NULL",
        )
