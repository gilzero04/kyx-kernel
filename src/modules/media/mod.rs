            ...
            web::scope("/media")
                ...
                .service(
                    web::resource("")
                        ...
                        .wrap(RequirePermission::new("media:upload", jwt_service.clone(), audit_service.clone()))
                        .route(web::post().to(interface::http::handlers::media::upload_file))
                )
                .service(
                    web::resource("")
                        .guard(web::guard::Get())
                        .wrap(RequirePermission::new("media:read", jwt_service.clone(), audit_service.clone()))
                        .route(web::get().to(interface::http::handlers::media::list_assets))
                )
                .service(
                    web::resource("/{id}")
                        .guard(web::guard::Delete())
                        .wrap(RequirePermission::new("media:delete", jwt_service.clone(), audit_service.clone()))
                        .route(web::delete().to(interface::http::handlers::media::delete_asset))
                )
                .service(
                    web::resource("/folders")
                        .guard(web::guard::Post())
                        .wrap(RequirePermission::new("media:folder", jwt_service.clone(), audit_service.clone()))
                        .route(web::post().to(interface::http::handlers::media::create_folder))
                )
                .route("/{tenant_id}/{filename}", web::get().to(interface::http::handlers::media::serve_file))
        );
