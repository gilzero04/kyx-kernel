    ...
    config.service(
        web::scope("/rbac")
            .service(
                web::scope("/roles")
                    .wrap(
                        RequirePermission::system("role:read", jwt.clone(), audit.clone())
            ...
            .service(
                web::scope("/permissions")
                    .wrap(
                        RequirePermission::system("permission:read", jwt.clone(), audit.clone())
                            .with_redis_opt(redis.clone()),
                    )
...
pub fn rbac_routes(
    ...
    config.service(
        web::scope("/roles")
            .wrap(
                RequirePermission::new("role:read", jwt.clone(), audit.clone())
                    .with_redis_opt(redis.clone()),
            )
            .route("", web::get().to(rbac::list_roles))
            .service(
                web::resource("")
                    .wrap(
                        RequirePermission::new("role:create", jwt.clone(), audit.clone())
                            .with_redis_opt(redis.clone()),
                    )
                    .route(web::post().to(rbac::create_role)),
            )
                ...
                web::scope("/{id}")
                    .service(
                        web::resource("")
                            ...
                            .route(web::patch().to(rbac::update_role))
                            .wrap(
                                RequirePermission::new("role:update", jwt.clone(), audit.clone())
                                    .with_redis_opt(redis.clone()),
                            ),
                    )
                    .service(
                        web::resource("")
                            ...
                            .route(web::delete().to(rbac::delete_role))
                            .wrap(
                                RequirePermission::new("role:delete", jwt.clone(), audit.clone())
                                    .with_redis_opt(redis.clone()),
                            ),
                    )
                    .service(
    ...
    config.service(
        web::scope("/permissions")
            .wrap(
                RequirePermission::new("permission:read", jwt.clone(), audit.clone())
                    .with_redis_opt(redis.clone()),
            )
            .route("", web::get().to(rbac::list_permissions))
            .service(
                web::resource("")
                    .wrap(
                        RequirePermission::new("permission:create", jwt.clone(), audit.clone())
                            .with_redis_opt(redis.clone()),
                    )
                    .route(web::post().to(rbac::create_permission)),
            )
