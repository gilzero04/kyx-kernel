...
pub async fn create_tenant(
    ...
    match service.create_tenant(
        ...
        Ok(data) => {
            let _ = audit.log("SuperAdmin", "TENANT_CREATED", Some(&data.id.to_string()), "SUCCESS", None).await;
...
pub async fn update_tenant(
    ...
    match service.update_tenant(
        ...
        Ok(data) => {
            let _ = audit.log("SuperAdmin", "TENANT_UPDATED", Some(&data.id.to_string()), "SUCCESS", None).await;
            web::HttpResponse::Ok().json(&data)
        },
        Err(e) => web::HttpResponse::InternalServerError().json(&serde_json::json!({
            "error": e.message
...
pub async fn delete_tenant(
    ...
    match service.delete_tenant(tenant_id, actor_tenant_id).await {
        Ok(_) => {
            let _ = audit.log("SuperAdmin", "TENANT_DELETED", Some(&tenant_id.to_string()), "SUCCESS", None).await;
            web::HttpResponse::Ok().json(&serde_json::json!({ "success": true }))
        },
...
pub async fn verify_domain(
    ...
    let custom_domain = body.get("custom_domain").and_then(|v| v.as_str()).unwrap_or("");

    if custom_domain.is_empty() {
        return web::HttpResponse::BadRequest().json(&serde_json::json!({ "error": "custom_domain is required" }));
    }
