...
mod tests {
    ...
    async fn test_hook_subscription_and_dispatch() {
        ...
        // Subscribe
        registry
            .subscribe(PluginHook::OnUserLogin, "test-plugin", handler)
            .await;
    ...
    async fn test_unsubscribe_all() {
        ...
        registry
            .subscribe(PluginHook::OnUserLogin, "plugin-1", handler.clone())
            .await;
        registry
            .subscribe(PluginHook::OnPaymentReceived, "plugin-1", handler.clone())
            .await;

        assert_eq!(registry.subscriber_count(PluginHook::OnUserLogin).await, 1);
        assert_eq!(
            registry
                .subscriber_count(PluginHook::OnPaymentReceived)
                .await,
            1
        );
