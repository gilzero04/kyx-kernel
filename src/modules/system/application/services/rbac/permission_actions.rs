...
pub async fn list(repo: &Arc<dyn RbacRepository>, actor_tenant_id: Option<Uuid>) -> Result<Vec<Permission>, AppError> {
    repo.list_permissions(None, actor_tenant_id).await.map_err(|e| AppError { code: 500, message: e.to_string() })
}

pub async fn create(repo: &Arc<dyn RbacRepository>, cmd: CreatePermissionCmd, actor_tenant_id: Option<Uuid>) -> Result<Permission, AppError> {
    if actor_tenant_id.is_some() {
        return Err(AppError { code: 403, message: "Only system owners can manage global permissions".into() });
    }
    repo.create_permission(cmd).await.map_err(|e| AppError { code: 500, message: e.to_string() })
}

pub async fn update(repo: &Arc<dyn RbacRepository>, id: Uuid, cmd: UpdatePermissionCmd, actor_tenant_id: Option<Uuid>) -> Result<Permission, AppError> {
    if actor_tenant_id.is_some() {
        return Err(AppError { code: 403, message: "Only system owners can manage global permissions".into() });
    }
