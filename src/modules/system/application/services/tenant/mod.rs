...
impl TenantService {
    ...
    pub async fn update_owner(
        ...
        self.repo.update_owner(name, slug, branding_id, contact_email, contact_phone, website_url, social_links, address, business_type, tax_id, config, custom_domain, allow_child_subdomains, domain_verified_at, verification_token).await.map_err(|e| AppError {
            code: 500,
            message: e.to_string(),
        })
    }
    ...
    pub async fn create_tenant(
        ...
        let tenant = self.repo.create(name, slug, type_slug, plan, admin_infos, parent_id, branding_id, contact_email, contact_phone, website_url, social_links, address, business_type).await.map_err(|e| AppError {
            code: 500,
            message: e.to_string(),
        })?;
    ...
    pub async fn update_tenant(
        ...
        self.repo.update_tenant(id, name, slug, is_active, branding_id, contact_email, contact_phone, website_url, social_links, address, business_type, config, actor_tenant_id, custom_domain, allow_child_subdomains, use_parent_subdomain, domain_verified_at, verification_token).await.map_err(|e| AppError {
            code: 500,
            message: e.to_string(),
        })
    }
