...
use std::fs;
use std::sync::Arc;
use wasmer::{Cranelift, Instance, Module, Store, imports};
...
impl WasmEngine {
    ...
    pub async fn load_from_path(&self, path: &str) -> Result<Instance> {
        ...
        let wasm_bytes =
            fs::read(path).map_err(|e| anyhow!("Failed to read WASM file at {}: {}", path, e))?;
    ...
    pub async fn load(
        &self,
        manifest: &crate::modules::system::domain::plugin::Manifest,
        path: &str,
    ) -> Result<Instance> {
        info!(
            "Loading WASM Plugin: {} (v{})",
