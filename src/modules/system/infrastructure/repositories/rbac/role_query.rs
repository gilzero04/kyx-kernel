use crate::modules::system::domain::rbac::{Role, CreateRoleCmd, UpdateRoleCmd};
use crate::core::infrastructure::database::Database;
use anyhow::{Result, anyhow};
use std::sync::Arc;
use uuid::Uuid;

pub async fn list(pool: &Arc<Database>) -> Result<Vec<Role>> {
    let roles = sqlx::query_as::<_, Role>(
        "SELECT id, code, slug, name, description, is_active, created_at, updated_at 
         FROM sys_roles WHERE deleted_at IS NULL ORDER BY sort_order ASC, name ASC"
    )
    .fetch_all(&pool.pool)
    .await
...
pub async fn update(pool: &Arc<Database>, id: Uuid, cmd: UpdateRoleCmd) -> Result<Option<Role>> {
    let role = sqlx::query_as::<_, Role>(
