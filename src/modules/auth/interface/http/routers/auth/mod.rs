...
pub fn auth_routes(
    ...
    web::scope("/auth")
        ...
        .service(web::resource("/refresh").route(web::post().to(auth::refresh_session)))
        .service(web::resource("/logout").route(web::post().to(auth::logout)))
        .service(web::resource("/setup/status").route(web::get().to(auth::get_setup_status)))
        .service(web::resource("/setup/verify-key").route(web::post().to(auth::verify_engine_key)))
        .service(
            web::resource("/setup/check-slug").route(web::get().to(auth::check_slug_availability)),
        )
        .service(web::resource("/setup").route(web::post().to(auth::initialize_system)))
        ...
        .service(
            web::scope("/admin")
                .wrap(admin_auth)
                .service(web::resource("/sessions").route(web::get().to(auth::admin_list_sessions)))
                .service(
