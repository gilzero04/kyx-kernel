...
TOKEN=$(echo "$RESP" | jq -r '.data.access_token // .access_token // empty')
...
# Re-login after session revoke (token was invalidated)
echo "Re-logging in after session revoke..."
LOGIN_AFTER_REVOKE=$(curl -s -X POST "$URL/api/v1/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"username":"admin@kyz.tech","password":"Admin123!"}')
TOKEN=$(echo "$LOGIN_AFTER_REVOKE" | jq -r '.data.access_token // empty')
echo "$TOKEN" > /tmp/kyx_token.txt
echo ""
...
SCRIPT_DIR="/Users/beykyu/Developer/kyx-tech/kyx-kernel/scripts"
...
AUDIT_LIST=$(curl -s -X GET "$URL/api/v1/admin/logs?limit=10" \
    -H "Authorization: Bearer $TOKEN")
...
if [ "$AUDIT_SUCCESS" = "true" ]; then
    echo "✅ GET /admin/logs success = true"
    PASS=$((PASS + 1))
else
    echo "❌ GET /admin/logs success = $AUDIT_SUCCESS"
    FAIL=$((FAIL + 1))
fi
